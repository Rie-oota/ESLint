...
$(function(){
  ...
  $('.search-btn').on('click', function(){
    ...
    // クエリパラメータを構築指定したURLから検索ワードの情報を取得
    const settings = {
      ...
      if (result[0].items?.length > 0) {
        ...
        $.each(result[0].items, function (getindex, getVal) {
          ...
          = $('<p>').html('出版社：' + (
            ...
            : '出版社不明')).prop('outerHTML');
          // リンク(getVal.link["@id"])の値が存在しているかを確認し、ある場合は値を表示、ない場合はテキストを定数へ代入
          const getLink
          = $('<a>').attr('href', getVal.link['@id']).attr('target', '_blank').text('書籍情報').prop('outerHTML');
          // 検索値を表示する
          const getResult
          = `<li class='lists-item'><div class='list-inner'>${getTitle}${getCreator}${getPublisher}${getLink}</div></li>`;
